...
<main class="container key-page">
    ...
    <div class="key-card">
        ...
        <c:choose>
            <c:when test="${activeKey != null}">
                ...
                <br/>
                <form action="key" method="POST" class="inline-form"
                      onsubmit="return confirm('Báo mất khóa sẽ thu hồi khóa hiện tại. Bạn cần tạo khóa mới. Tiếp tục?')">
                    <input type="hidden" name="action" value="revoke"/>
                    <button type="submit" class="btn btn-danger">Báo mất khóa / Thu hồi</button>
                </form>
            </c:when>
            <c:otherwise>
                <p class="text-muted">Bạn chưa có khóa nào. Hãy tạo khóa hoặc nhập public key cá nhân để ký đơn hàng.</p>
            </c:otherwise>
        </c:choose>
    </div>
    ...
    <div class="key-card">
        ...
        <p class="text-muted">Hệ thống sẽ tạo khóa mới và bạn hãy lưu khóa của mình lại. <strong>Lưu giữ cẩn thận và không chia sẻ khóa với ai.</strong></p>
        <c:choose>
            <c:when test="${activeKey != null}">
                <p class="text-muted"> <strong> Bạn hãy báo mất/thu hồi khóa hiện tại trước khi tạo khóa mới.</strong></p>
                <button type="button" class="btn btn-muted" disabled>Khóa của bạn đang còn hoạt động</button>
            </c:when>
            <c:otherwise>
                <form action="key" method="POST">
                    <input type="hidden" name="action" value="generate"/>
                    <button type="submit" class="btn btn-primary">Tạo cặp khóa RSA 2048-bit</button>
                </form>
            </c:otherwise>
        </c:choose>
    </div>
    ...
    <div class="key-card">
        ...
        <div class="own-key-layout">
            <div>
                <p class="text-muted">Bạn đã có cặp khóa riêng, hãy dán public key RSA vào đây. Bạn dùng private key tương ứng của mình để ký đơn hàng.</p>
                <c:choose>
                    <c:when test="${activeKey != null}">
                        <p class="text-muted"><strong>Bạn hãy báo mất/thu hồi khóa hiện tại trước khi thay bằng public key cá nhân.</strong></p>
                        <button type="button" class="btn btn-muted" disabled>Khóa của bạn đang còn hoạt động</button>
                    </c:when>
                    <c:otherwise>
                        <form action="key" method="POST">
                            ...
                            <div class="own-key-action-row">
                                <button type="submit" class="btn btn-secondary">Sử dụng public key này</button>
                                <div class="own-key-import-row">
                                    <span id="ownPublicKeyFileName" class="own-key-file-name"></span>
                                    <button type="button" class="btn btn-secondary" onclick="document.getElementById('ownPublicKeyFile').click()">
            ...
            <div class="own-key-note">
                <strong>Lưu ý:</strong>
                <p class="mb-0">Hệ thống chỉ lưu public key để xác minh chữ ký.</p>
            </div>
        </div>
    </div>
    ...
    <div class="key-choice-overlay" role="dialog" aria-modal="true" aria-labelledby="keyChoiceTitle">
        <div class="key-choice-dialog">
            ...
            <div class="key-choice-grid">
                <div class="key-choice-box">
                    ...
                    <form action="key" method="POST">
                        ...
                        <button type="submit" class="btn btn-primary">Lưu file khóa</button>
                    </form>
                </div>
                ...
                <div class="key-choice-box">
                    ...
                    <div class="gmail-row">
                        ...
                        <button type="button" class="btn btn-secondary" onclick="openGmailDraft()">Gửi mail</button>
                    </div>
            ...
            <form action="key" method="POST" class="key-choice-actions">
                ...
                <button type="submit" class="btn btn-secondary">Đóng</button>
            </form>
...
<script>
    ...
    function openGmailDraft() {
        ...
        const body = 'Private key của bạn:\n\n' + privateKey;
